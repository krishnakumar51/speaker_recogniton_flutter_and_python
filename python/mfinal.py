...
def get_embedding(audio_path):
    ...
    signal = torch.tensor(np.expand_dims(signal, axis=0), device=device)
...
def augment_data(signal, sr):
    ...
    noise = np.random.randn(len(signal)) * 0.005  # Add small random noise
...
def collect_embeddings(folder, label, augment=False):
    ...
    embeddings = []
    labels = []
    if not os.listdir(folder):  # Handle empty folder case
    ...
    for file_name in os.listdir(folder):
        if file_name.endswith('.wav'):
            ...
            if augment:
                ...
                augmented_embedding = spk_rec_model.encode_batch(torch.tensor(np.expand_dims(augmented_signal, axis=0), device=device))
...
def prepare_data():
    ...
    if not authorized_embeddings and not unauthorized_embeddings:  # Handle case where no data is available
        raise ValueError("No valid audio data found in either authorized or unauthorized folders.")
...
def train_model():
    ...
    model = SpeakerRecognitionModel(input_size).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(len(X_train)):
            optimizer.zero_grad()
            output = model(torch.tensor(X_train[i], dtype=torch.float32, device=device))
            loss = criterion(output, torch.tensor([y_train[i]], dtype=torch.float32, device=device))
            loss.backward()
            optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i in range(len(X_test)):
            output = model(torch.tensor(X_test[i], dtype=torch.float32, device=device))
            pred = (output > 0.5).float()
            correct += (pred.cpu() == y_test[i]).sum().item()
...
def recognize_speaker(audio_path, model):
    ...
    model.eval()
    with torch.no_grad():
        output = model(torch.tensor(embedding, dtype=torch.float32, device=device))
